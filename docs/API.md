...
VolScope exposes a local HTTP API on the configured loopback port.
...
`POST /api/copilot/query` answers a bounded question about the authoritative
snapshot named by the request. The server reloads replay or live state instead
of trusting browser-supplied analytics.
...
V0 is deterministic and read-only. Responses contain an answer with `[E#]`
references, a structured evidence list, snapshot provenance, and explicit
limitations. It does not submit orders or provide strategy recommendations.
