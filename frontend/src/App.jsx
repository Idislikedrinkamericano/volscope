...
function App() {
  ...
  const [layout, setLayout] = useState(() => localStorage.getItem('volscope-layout') || 'dense')
  const [workspaces, setWorkspaces] = useState(() => {
    try {
      const value = JSON.parse(localStorage.getItem('volscope-workspaces') || '[]')
  ...
  useEffect(() => {
    localStorage.setItem('volscope-layout', layout)
  }, [layout])
  ...
  const saveWorkspace = () => {
    ...
    localStorage.setItem('volscope-workspaces', JSON.stringify(next))
  }
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="brand"><Activity size={20} /><strong>VolScope</strong><span>Rust Analytics</span></div>
      ...
      <main className={`research-workspace layout-${layout}`}>
        ...
        <CopilotPanel
          context={{ mode, symbol: activeSymbol, date: tradingDate, minute, expiration, pricing_mode: pricingMode, dealer_model: dealerModel }}
          disabled={!chain || !minute || !expiration}
