...
![VolScope 实时分析界面](frontend/public/guide/workbench-overview.png)
...
工作台包含一个只读、证据约束的研究对话面板。它会在 Rust 后端重新读取
当前 replay/live 截面，并回答波动率、dealer exposure 和数据质量问题。
所有数值引用均指向结构化 `[E#]` 证据；V0 不调用外部模型、不推荐交易、
也不具备下单权限。
...
VolScope 不输出简单的“买入”或“卖出”按钮。它把一次期权判断拆成七个
可以被检查和推翻的步骤：
...
如果有历史数据，修改 `.env` 中的 `VOLSCOPE_DATA_ROOT`，然后：
...
2. 服务端显式设置 `VOLSCOPE_PAPER_ORDER_EXECUTION=1`；
...
| 变量 | 默认值 | 作用 |
| --- | --- | --- |
| `VOLSCOPE_DATA_ROOT` | `./data` | 历史正股与期权分区 |
| `VOLSCOPE_HOST` | `127.0.0.1` | HTTP/WebSocket 监听地址 |
| `VOLSCOPE_PORT` | `7311` | 服务端口 |
| `VOLSCOPE_RISK_FREE_RATE` | `0.043` | BSM 无风险利率 |
| `VOLSCOPE_FRONTEND_DIST` | `./frontend/dist` | 前端构建目录 |
| `VOLSCOPE_AUDIT_PATH` | `~/.volscope/audit.jsonl` | 追加式审计记录 |
| `VOLSCOPE_PAPER_ORDER_EXECUTION` | 未设置 | 模拟下单服务端总开关 |
| `RUST_LOG` | `volscope=info,tower_http=info` | 日志过滤器 |
