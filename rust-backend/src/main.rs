...
async fn copilot_query(
    State(state): State<AppState>,
    Json(request): Json<CopilotRequest>,
) -> Result<Json<Value>, ApiError> {
    request.validate().map_err(ApiError::bad_request)?;
    let (chain, volatility) = if request.mode == "live" {
        ...
        if !request.symbol.eq_ignore_ascii_case(&snapshot.chain.symbol) {
            return Err(ApiError::conflict(
                "live symbol changed; refresh the copilot context",
            ));
        }
        let closes = state.live.daily_closes(45).await.map_err(ApiError::upstream)?;
        let volatility = state
            .replay
            .live_volatility_context(&snapshot.chain, &closes)
            .map_err(ApiError::bad_request)?;
        (snapshot.chain, volatility)
    } else {
        let date = request.date.as_deref().ok_or_else(|| ApiError::bad_request("date is required for replay copilot"))?;
        let minute = request.minute.as_deref().ok_or_else(|| ApiError::bad_request("minute is required for replay copilot"))?;
        let expiration = request.expiration.as_deref().ok_or_else(|| ApiError::bad_request("expiration is required for replay copilot"))?;
        validate_minute(minute)?;
        let chain = state.replay.chain(
            &request.symbol,
            date,
            minute,
            expiration,
        ...
        let volatility = state.replay.volatility_context(
            &request.symbol,
            date,
            minute,
            expiration,
    ...
    serde_json::to_value(copilot::answer(&request.question, &chain, &volatility))
        .map(Json)
        .map_err(ApiError::bad_request)
}
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "volscope=info,tower_http=info".into()),
        )
    ...
    let project_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .unwrap()
        .to_path_buf();
    let data_root = env::var("VOLSCOPE_DATA_ROOT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| project_root.join("data"));
    let frontend_dist = env::var("VOLSCOPE_FRONTEND_DIST")
        .map(PathBuf::from)
        .unwrap_or_else(|_| project_root.join("frontend/dist"));
    let risk_free_rate = env::var("VOLSCOPE_RISK_FREE_RATE")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(0.043);
    let host = env::var("VOLSCOPE_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
    let port: u16 = env::var("VOLSCOPE_PORT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(7311);
    let live = LiveManager::new(risk_free_rate);
    live.start_refresh_loop();
    let audit_path = env::var("VOLSCOPE_AUDIT_PATH")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
    ...
    tracing::info!(%address, "VolScope Rust service listening");
